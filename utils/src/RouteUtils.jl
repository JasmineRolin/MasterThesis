...
function checkRouteFeasibility(scenario::Scenario,vehicleSchedule::VehicleSchedule)
    ...
    if activeTimeWindow.startTime != route[1].startOfServiceTime || activeTimeWindow.endTime != route[end].endOfServiceTime
        error("ROUTE INFEASIBLE: Active time window of vehicle $(vehicle.id) is incorrect")
        return false
    end
    ...
    if activeTimeWindow.startTime < vehicle.availableTimeWindow.startTime || activeTimeWindow.endTime > vehicle.availableTimeWindow.endTime
        error("ROUTE INFEASIBLE: Vehicle $(vehicle.id) is not available during the route")
        return false
    end
    
    # Check maximum route duration 
    if activeTimeWindow.endTime - activeTimeWindow.startTime > vehicle.maximumRideTime
        error("ROUTE INFEASIBLE: Vehicle $(vehicle.id) exceeds maximum ride time")
        return false
    end
    ...
    for (idx,activityAssignment) in enumerate(route[2:end-1]) # Do not check depots
        ...
        if activity.id in hasBeenServiced
            error("ROUTE INFEASIBLE: Activity $(activity.id) visited more than once on vehicle $(vehicle.id)")
            return false
        else
        ...
        elseif activity.activityType == DROPOFF 
            pickUpId = findCorrespondingId(activity.id,nRequests)
            ...
            if rideTime > scenario.requests[activity.requestId].maximumRideTime || rideTime < scenario.requests[activity.requestId].directDriveTime
                error("ROUTE INFEASIBLE: Maximum ride time exceeded for drop-off $(activity.id) on vehicle $(vehicle.id)")
            end
        ...
        if startOfServiceTime < activity.timeWindow.startTime || endOfServiceTime > activity.timeWindow.endTime
            error("ROUTE INFEASIBLE: Time window not respected for activity $(activity.id) on vehicle $(vehicle.id). 
                   ...
                   Time Window: ($(activity.timeWindow.startTime), $(activity.timeWindow.endTime))")
        end
        ...
        if startOfServiceTime < route[idx-1].endOfServiceTime + scenario.time[route[idx-1].activity.id,activity.id]
            error("ROUTE INFEASIBLE: Start of service time $(startOfServiceTime) of activity $(activity.id) is not correct")
            return false
        end
        if endOfServiceTime != startOfServiceTime + scenario.serviceTimes[activity.mobilityType]
            error("ROUTE INFEASIBLE: End of service time $(endOfServiceTime) of activity $(activity.id) is not correct")
            return false
        end
        ...
        if activity.mobilityType == WHEELCHAIR
            ...
            if currentCapacities[WHEELCHAIR] > vehicle.capacities[WHEELCHAIR] || currentCapacities[WHEELCHAIR] < 0
                error("ROUTE INFEASIBLE: Capacities exceeded for vehicle $(vehicle.id)")
                return false
            end
        else
        ...
        if currentCapacities[WHEELCHAIR] != numberOfWheelchair[idx] || currentCapacities[WALKING] != numberOfWalking[idx]
            error("ROUTE INFEASIBLE: Capacities not updated correctly for vehicle $(vehicle.id)")
            return false
        end
        
        # Keep track of total distance and total time 
        totalDistanceCheck += scenario.distance[route[idx-1].activity.id,activity.id]
        totalDurationCheck += scenario.time[route[idx-1].activity.id,activity.id]
    end

    # Add end depot to total distance and duration
    totalDistanceCheck += scenario.distance[route[end-1].activity.id,route[end].activity.id]
    totalDurationCheck += scenario.time[route[end-1].activity.id,route[end].activity.id]

    # Check that total distance and total duration are correct
    if totalDistanceCheck != totalDistance
        error("ROUTE INFEASIBLE: Total distance $(totalDistance) is incorrect")
        return false
    end
