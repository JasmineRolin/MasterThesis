...
mutable struct VehicleSchedule 
    ...
    function VehicleSchedule(vehicle::Vehicle)
        ...
        return new(vehicle, [startDepot,endDepot], TimeWindow(0, 0), 0.0, 0, 0.0, Int[0,0], Int[0,0]) 

    end

    function VehicleSchedule(vehicle::Vehicle, route::Vector{ActivityAssignment} )
        return new(vehicle, route, TimeWindow(0, 0), 0.0, 0, 0.0, Int[], Int[]) 
    end

    function VehicleSchedule()
        return new(Vehicle(), [], TimeWindow(0, 0), 0.0, 0, 0.0, Int[], Int[])
    end 
