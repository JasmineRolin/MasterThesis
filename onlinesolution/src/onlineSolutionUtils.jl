...
==#
function onlineAlgorithm(currentState::State, requestBank::Vector{Int}, scenario::Scenario, destroyMethods::Vector{GenericMethod}, repairMethods::Vector{GenericMethod};ALNS::Bool=true, nNotServicedExpectedRequests::Int=0,splitRequestBank::Bool=true,ALNSparametersOnline::String="tests/resources/ALNSParameters_online.json")
    ...
    if ALNS
        finalSolution,finalOnlineRequestBank = runALNS(scenario, scenario.requests, destroyMethods,repairMethods;parametersFile=ALNSparametersOnline,initialSolution =  currentSolution, requestBank = newRequestBankOnline, event = event, alreadyRejected =  totalNTaxi, visitedRoute = currentState.visitedRoute,stage = "Online", nNotServicedExpectedRequests=nNotServicedExpectedRequests,splitRequestBank=splitRequestBank)
    else
